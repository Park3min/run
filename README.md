...
- **기록 표시** — 총 거리·평균 페이스(자동 계산)·시간·케이던스·평균 심박수를 카운트업 애니메이션으로 표시
- **구간(스플릿) 기록** — 1km 페이스 또는 랩 단위 바 차트
- **사진에 기록 새기기** — 사진 위에 기록 오버레이 (그라데이션/미니멀/카드 스타일, 드래그로 위치 조정), PNG 저장 및 인스타 스토리(9:16) 내보내기
- **영상 캡처 모드** — 검정/크로마키/흰색 빈 배경에서 카운트업만 재생 (화면 녹화용)
- **가민 커넥트 연동** — 로컬 서버로 최근 활동을 불러와 자동 입력 (활동명·날짜·랩 포함)
- **가민 CSV 가져오기** — 가민 커넥트에서 내보낸 활동 CSV 파싱 (한국어/영어 헤더 지원)
